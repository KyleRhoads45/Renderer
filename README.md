...
### Features
---
* [Physically Based Rendering Shader](#physically-based-shader)
...
###### Entity
An entity is a class whose only field is an ID, which is needed to fetch its components from certain component pools. The reason for it being a class and not just a typedef is for the added helper functions that wrap around the registry API.
