...
Entity ProcessNode(const aiScene* scene, const aiNode* node);
...
Entity InstantiateModel(const char* meshPath) {
	...
	return ProcessNode(scene, scene->mRootNode);
}
...
Entity ProcessNode(const aiScene* scene, const aiNode* node) {
	...
	transform->childCount = node->mNumChildren;
	transform->children = std::make_shared<Transform*[]>(transform->childCount);
	for (unsigned int i = 0; i < node->mNumChildren; i++) {
		auto child = ProcessNode(scene, node->mChildren[i]);
		transform->children[i] = child.GetComponent<Transform>();
	}
